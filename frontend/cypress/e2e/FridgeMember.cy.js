Cypress.on("uncaught:exception", (err, runnable) => {
  ...
  // failing the test on unhandled promise rejections
  if (
    err.message.includes("Cannot read properties of undefined (reading 'type')")
  ) {
    return false;
  }
});
...
describe("Testing frdige member management", () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept("GET", `${base_url_endpoint}/user/search/johnbeef`, (req) => {
      ...
      req.reply({
        statusCode: 200,
        body: [
          { userId: 11, username: "johnbeef" },
          { userId: 52, username: "atle" },
          {
            userId: 82,
            username: "leeet",
          },
          { userId: 84, username: "kirol123" },
          { userId: 86, username: "leonhest" },
        ],
      });
    }).as("fetchUser");
    ...
    cy.intercept("POST", "http://localhost:8089/api/fridge/add/user", (req) => {
      req.reply({
        statusCode: 200,
      });
  ...
  it("should be able remove yourself from a fridge", () => {
    cy.contains(".member", "Super bruker").find(".fa-trash").click();
